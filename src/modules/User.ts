...
export namespace User {
  ...
  class group extends ApiController {
    ...
    list(
      ...
      P: number = 1,
      N: number = 999
    ): Promise<SearchResult<ClassUserGroup>> {
  ...
  class auth extends ApiController {
    ...
    async regist(
      ...
      return await this._post("regist", {
        Name,
        Nick,
        Sex,
        Account,
  ...
  class users extends ApiController {
    ...
    search(
      W: any,
      conf: { Keyword?: string; N?: number; P?: number; Sort?: string }
    ) {
      if (W === void 0) {
        W = {};
      }
      if (undefined === conf) {
        conf = { N: 10, P: 1, Keyword: "" };
      }
  ...
  class rule extends ApiController {
    ...
    async group(): Promise<{ [index: string]: RuleGroupClass }> {
      let rs: { [index: string]: RuleGroupClass } = {},
